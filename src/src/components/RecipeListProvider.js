...
function RecipeListProvider({ children }) {
  ...
  async function handleLoad() {
    ...
    const response = await fetch(`http://localhost:3000/recipe/list`, {
      method: "GET",
    });
  ...
  async function handleUpdate(dtoIn) {
    setRecipeLoadObject((current) => ({ ...current, state: "pending" }));
    const response = await fetch(`http://localhost:3000/recipe/update`, {
      method: "POST",
  ...
  async function handleDelete(dtoIn) {
    setRecipeLoadObject((current) => ({ ...current, state: "pending" }));
    const response = await fetch(`http://localhost:3000/recipe/delete`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(dtoIn),
    });
